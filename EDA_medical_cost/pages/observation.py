...
def observation():
    st.markdown(f""" 
                ## Observation
                In this page, we give some interesting observations on our data. 
                """)
    ...
    with st.expander("Which region has the highest average charges?"):
        ...
        st.plotly_chart(region_avg_bar_fig)
    ...
    with st.expander("Person with the highest charges in each region"):
        high_charge_in_region()
        
    with st.expander("Person with the lowest charges in each region"):
        low_charge_in_region()
        
    with st.expander("Correlation of each feature (heatmap)"):
        draw_heatmap()
